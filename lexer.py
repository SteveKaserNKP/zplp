...
if __name__ == "__main__":
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith("DEF"):
            ...
            lex.append(Def_Line(Line_Type.DEF, line, i+1, var, value))
            ...
            lex.append(Block_Begin_Line(Line_Type.BLOCK_BEGIN, line, i+1, block_type, label))
        elif line.startswith("END_"):
            if line.startswith("END_DOCUMENT"):
                block_type = Block_Type.DOCUMENT
            elif line.startswith("END_SETUP"):
                block_type = Block_Type.SETUP
            ...
            if ":" in line:
                label = ''.join(line.split(":")[1:])
            lex.append(Block_End_Line(Line_Type.BLOCK_END, line, i+1, block_type, label))
        elif line.startswith("//"):
            lex.append(Line(Line_Type.COMMENT, line, i+1))
        else:
            first_space = line.index(" ")
            param = line[0:first_space]
            value = line[first_space+1:]
            lex.append(Parameter_Line(Line_Type.PARAMETER, line, i+1, param, value))
